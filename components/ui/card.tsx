...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-4 sm:gap-6 rounded-lg border border-border/60 py-4 sm:py-6 shadow-sm transition-all duration-200 hover:shadow-md hover:border-border",
        className
      )}
      {...props}
    />
