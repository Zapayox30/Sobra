...
import { calculateExpenseDistribution } from '@/lib/chart-utils'
...
export default function AnalyticsPage() {
  ...
  const {
    surplus,
    isLoading,
  } = useSurplus(selectedMonth)

  const { data: surplusHistory = [] } = useSurplusHistory(12)

  const currency = useMemo(() => profile?.currency || 'PEN', [profile])

  // Build trend data from surplus_history (real multi-month data)
  const trendData: TrendPoint[] = useMemo(() => {
    if (surplusHistory.length > 0) {
      return surplusHistory.map((h) => {
        const totalExpenses =
          Number(h.fixed_expenses_total) +
          Number(h.commitments_total) +
          Number(h.personal_expenses_total) +
          Number(h.card_payments_total)
        return {
          month: format(new Date(h.month + '-01'), 'MMM yyyy'),
          income: Number(h.income_total),
          expenses: totalExpenses,
          leftover: Number(h.net_surplus),
        }
      })
    }
    // Fallback: show current month only from live calculation
    if (!surplus) return []
    return [{
      month: format(selectedMonth, 'MMM yyyy'),
      income: surplus.incomeTotal,
      expenses: surplus.fixedTotal + surplus.commitmentsTotal + surplus.personalTotal + surplus.cardDueTotal,
      leftover: surplus.netSurplus,
    }]
  }, [surplusHistory, surplus, selectedMonth])

  const isEmpty =
    surplus &&
    surplus.incomeTotal === 0 &&
    surplus.fixedTotal === 0 &&
    surplus.personalTotal === 0 &&
    surplus.commitmentsTotal === 0 &&
    surplus.cardDueTotal === 0

  const displayCalculation = showDemo ? demoCalculation : (
    surplus ? {
      incomeTotal: surplus.incomeTotal,
      fixedTotal: surplus.fixedTotal,
      commitmentsTotal: surplus.commitmentsTotal,
      personalTotal: surplus.personalTotal,
      cardDueTotal: surplus.cardDueTotal,
      leftoverBeforePersonal: surplus.grossSurplus,
      leftoverAfterPersonal: surplus.netSurplus,
      dailySuggestion: surplus.dailySuggestion,
      daysInMonth: surplus.daysInMonth,
      remainingDays: surplus.remainingDays,
    } : null
  )
