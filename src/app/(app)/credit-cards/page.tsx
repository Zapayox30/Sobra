...
import {
  useCreditCards, useCreateCreditCard, useUpdateCreditCard, useDeleteCreditCard,
  useCardStatements, useCreateCardStatement, useDeleteCardStatement,
  useCreateCardPayment, useDeleteCardPayment,
  useCardDues, useCardSpending, useCreateCardTransaction, useDeleteCardTransaction,
} from '@/hooks/use-credit-cards'
import { useProfile } from '@/hooks/use-user'
import { calculateInstallmentPlan, formatCurrency } from '@/lib/calc'
...
export default function CreditCardsPage() {
  ...
  const { data: statements = [] } = useCardStatements()
  const {
    cardDueTotal,
    cardMinimumDue,
    nextDueDate,
  ...
  const deleteStatement = useDeleteCardStatement()
  ...
  const handleUpdateCard = async (e: React.FormEvent) => {
    ...
    await updateCard.mutateAsync({
      ...
      credit_limit: editCardForm.credit_limit ? parseFloat(editCardForm.credit_limit) : null,
      cutoff_day: Number(editCardForm.cutoff_day),
      due_day: Number(editCardForm.due_day),
  ...
  const handleDeleteCard = async (id: string) => {
    if (!confirm('¿Eliminar esta tarjeta? Se borrarán estados, pagos y consumos asociados.')) return
    ...
    <div className="space-y-6">
              ...
              >
                ...
                {statements.map((st) => (
                  <option key={st.id} value={st.id}>
                    {st.statement_month?.slice(0, 10)} - vence {st.due_date ? new Date(st.due_date).toLocaleDateString() : 'N/A'}
                  </option>
                ))}
              </select>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive shrink-0" onClick={() => handleDeleteTransaction(tx.id)}>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Vence {st.due_date ? new Date(st.due_date).toLocaleDateString() : 'N/A'} • Mes {new Date(st.statement_month).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive shrink-0" onClick={() => handleDeleteStatement(st.id)}>
                    <Trash2 className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </div>
            ...
            <div key={card.id} className="rounded-xl border border-border/70 bg-card/80 p-4 space-y-2">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleDeleteCard(card.id)}>
                    <Trash2 className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </div>
      ...
      <Dialog open={!!editingCard} onOpenChange={(open) => !open && setEditingCard(null)}>
        <DialogContent>
          ...
          <form className="space-y-4" onSubmit={handleUpdateCard}>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <Label>Nombre</Label>
                <Input
                  value={editCardForm.name}
                  onChange={(e) => setEditCardForm((s) => ({ ...s, name: e.target.value }))}
                  required
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditCardForm((s) => ({ ...s, issuer: e.target.value }))}
                  placeholder="Opcional"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditCardForm((s) => ({ ...s, credit_limit: e.target.value }))}
                  placeholder="Opcional"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    min={1}
                    max={28}
                    value={editCardForm.cutoff_day}
                    onChange={(e) => setEditCardForm((s) => ({ ...s, cutoff_day: Number(e.target.value) }))}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditCardForm((s) => ({ ...s, due_day: Number(e.target.value) }))}
                    required
                  />
                </div>
              </div>
            ...
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="edit-card-active"
                checked={editCardForm.is_active}
                onChange={(e) => setEditCardForm((s) => ({ ...s, is_active: e.target.checked }))}
                className="h-4 w-4 rounded border-border"
              />
              <Label htmlFor="edit-card-active">Activa</Label>
            </div>
            <div className="flex gap-2 justify-end">
              ...
              <Button type="submit" disabled={updateCard.isPending}>
                {updateCard.isPending ? 'Guardando...' : 'Guardar cambios'}
              </Button>
            </div>
          </form>
        </DialogContent>
