...
import { EXPENSE_CATEGORIES, FIXED_EXPENSE_CATEGORIES } from '@/lib/categories'
...
import type { FixedExpense, FixedExpenseInsert, FixedExpenseUpdate, PersonalExpense, PersonalExpenseInsert, PersonalExpenseUpdate } from '@/types'
...
export function ExpenseForm({ type, expense, onSuccess }: ExpenseFormProps) {
  ...
  const form = useForm({
    ...
    defaultValues: expense
      ? {
          category: (expense as FixedExpense | PersonalExpense).category || 'otros',
          label: expense.label || '',
          amount: Number(expense.amount),
          recurrence: expense.recurrence as FixedExpenseInput['recurrence'],
  ...
  const onSubmit = async (data: FixedExpenseInput | PersonalExpenseInput) => {
    // serializeDates converts Date → string for Supabase compatibility
    const payload = serializeDates(data) as Record<string, unknown>
    if (expense) {
      if (isPersonal) {
        await updatePersonal.mutateAsync({ id: expense.id, ...payload } as PersonalExpenseUpdate & { id: string })
      } else {
        await updateFixed.mutateAsync({ id: expense.id, ...payload } as FixedExpenseUpdate & { id: string })
      }
    } else {
      if (isPersonal) {
        await createPersonal.mutateAsync(payload as Omit<PersonalExpenseInsert, 'user_id'>)
      } else {
        await createFixed.mutateAsync(payload as Omit<FixedExpenseInsert, 'user_id'>)
      }
    }
    onSuccess?.()
  }
